...
/**
 * 程序中尽可能使用单例 OkHttpClient，也就是多个 HTTP 请求尽可能使用同一个 OkHttpClient 变量， 屡次 new OkHttpClient 会致使抛出 too many
 * ope files 的异常，这个异常是由于多个 OkHttpClient 链接了多个 socket 致使的。如果有需要，可以使用 OkHttpClient 的 newBuilder()
 * 方法，再进行自定义。
 */
