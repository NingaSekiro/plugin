...
/**
 * 进程选择对话框组件 使用 JBTabbedPane 分成两个面板： - 本地进程：从提供的进程列表中选择 - 远程 JVM：输入 IP 地址（格式：IP:端口）
 * 确定后返回选择的进程字符串（本地）或 IP 字符串（远程），否则 null 只能选择一个面板的值
 */
public class JvmProcessSelectorDialog extends DialogWrapper {
  ...
  public JvmProcessSelectorDialog(List<String> processList) {
    ...
    setTitle(I18nUtil.message("jvm.selector.dialog.title"));
    initValidation();
    init();
    setupUI();
  }
  ...
  private void setupUI() {
    // 本地进程面板
    JBLabel localLabel = new JBLabel(I18nUtil.message("jvm.selector.label.local.process"));
    ...
    JBLabel remoteLabel = new JBLabel(
        I18nUtil.message("jvm.selector.label.remote.process"));
    ...
    // 添加到 TabbedPane
    tabbedPane.addTab(I18nUtil.message("jvm.selector.tab.local"), localPanel);
    tabbedPane.addTab(I18nUtil.message("jvm.selector.tab.remote"), remotePanel);
  }
  ...
  protected void doOKAction() {
    ...
    switch (selectedIndex) {
      case 0 -> { // 本地进程
        selectedValue = new HttpServer((String) localComboBox.getSelectedItem(), "127.0.0.1",
            12345);
      }
      case 1 -> { // 远程 JVM
        selectedValue = new HttpServer(null, remoteTextField.getText().split(":")[0],
            Integer.parseInt(remoteTextField.getText().split(":")[1]));
      }
    }
    super.doOKAction();
  }
    ...
    switch (selectedIndex) {
      case 0 -> {
        if (localComboBox.getSelectedItem() == null) {
          return new ValidationInfo(I18nUtil.message("jvm.selector.error.select.local"));
        }
        return null;
      }
      case 1 -> {
        ...
        if (serverName == null || serverName.trim().isEmpty()) {
          return new ValidationInfo(I18nUtil.message("jvm.selector.error.input.ip.port"));
        } else {
          String[] parts = serverName.split(":");
          if (parts.length != 2) {
            return new ValidationInfo(I18nUtil.message("jvm.selector.error.invalid.format"));
          }
          String ip = parts[0];
          String port = parts[1];
      ...
      default -> {
        return new ValidationInfo(I18nUtil.message("jvm.selector.error.select.panel"));
      }
    }
  }
